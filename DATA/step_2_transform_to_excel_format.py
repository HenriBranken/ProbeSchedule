...
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
writer = pd.ExcelWriter("cultivar_data.xlsx", engine="xlsxwriter", date_format="%Y-%m-%d")

for p in probe_ids:
    print("Currently busy with probe {:s}.".format(p))
    sub_df = pd.read_csv(p + "_daily_data.csv", sep=",", header=0, parse_dates=True, index_col=0,
                         na_values=["nan", "None"])
    # print(sub_df.info())
    # print(sub_df.tail())
    sub_df.to_excel(writer, header=True, index=True, sheet_name=p, index_label="date", verbose=True)
