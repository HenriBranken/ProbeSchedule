...
# ----------------------------------------------------------------------------------------------------------------------
# Let us rather start from the beginning, and import the daily_averaged_kcp data from the `day_frequency` sheet in the
# `binned_kcp_data.xlsx` excel file.  This way, this module incorporates the entire process from beginning to end.
binned_kcp_data_df = pd.read_excel("./data/binned_kcp_data.xlsx", sheet_name="day_frequency", header=0, index_col=0,
                                   parse_date=True)

with open("./data/starting_year.txt", "r") as f:
    starting_year = int(f.readline().rstrip())

# Remove season_day 366, which is datetime.datetime(year=starting_year+1, month=BEGINNING_MONTH, day=1).
to_check_for_date = datetime.datetime(year=starting_year+1, month=BEGINNING_MONTH, day=1)
...
last_day_of_the_month = calendar.monthrange(year=starting_year+1, month=BEGINNING_MONTH-1)[1]
ending_date = datetime.datetime(year=starting_year+1, month=BEGINNING_MONTH-1, day=last_day_of_the_month)
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++


# ======================================================================================================================
# Create weekly-averaged kcp data with weekly bins.  The dependent variable ranges in the interval [1; 52].
# ======================================================================================================================
sweek_kcps_dict = {k: [] for k in np.arange(start=1, stop=52+1, step=1)}  # Dictionary comprehension
...
# ----------------------------------------------------------------------------------------------------------------------
datetime_stamp = pd.date_range(start=starting_date, end=ending_date, freq="D", normalize=True)
projected_kcp = []
projected_week = []

for i in range(x_limits[1]):
    projected_week.append(min((i//7) + 1, 52))
    projected_kcp.append(sweek_kcps_dict.get(projected_week[i], sweek_kcps_dict[52]))
    print("i = {:>3}, s_day = {:>3}, s_week = {:>2},"
          " kcp = {:.4f}, datetime_stamp = {}.".format(i, season_day[i], projected_week[i], projected_kcp[i],
                                                       datetime_stamp[i].strftime("%Y-%m-%d")))

dict_for_df = {"projected_week": projected_week, "season_day": season_day, "projected_kcp": projected_kcp}
...
ax.plot(projected_df["season_day"].values, projected_df["projected_kcp"].values, lw=2)
...
# ======================================================================================================================
projected_df.to_excel("./data/projected_weekly_data.xlsx", float_format="%.7f", header=True, index=True,
