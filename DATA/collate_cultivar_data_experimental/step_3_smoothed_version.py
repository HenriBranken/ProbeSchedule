...
# ----------------------------------------------------------------------------------------------------------------------
n_neighbours_list = list(np.arange(start=30, stop=1-1, step=-1))  # i.e. the width of the Gaussian in the gaussian
# helper function.
...
try:
    ...
    x_smoothed, y_smoothed = get_final_polynomial_fit(x_raw=independent_var, y_raw=dependent_var, step_size=delta_x,
                                                      degree=pol_degree, x_lims=x_limits)
    r_squared_stats.append(get_r_squared(x_raw=independent_var, y_raw=dependent_var, x_fit=x_smoothed,
                                         y_fit=y_smoothed))

# We need to safe-check the reasonableness of the WMA trendline if mode is still equal to "WMA".
if mode == "WMA":
    print("We are still in \"WMA\" mode.")
    r_squared_check = get_r_squared(x_raw=cco_df["days"].values, y_raw=cco_df["cco"].values,
                                    x_fit=x_smoothed, y_fit=y_smoothed)
    print("r_squared_check = {:.4f}.".format(r_squared_check))

    x_pol, y_pol = get_final_polynomial_fit(x_raw=independent_var, y_raw=dependent_var, step_size=delta_x,
                                            degree=pol_degree, x_lims=x_limits)
    r_squared_pol = get_r_squared(x_raw=cco_df["days"].values, y_raw=cco_df["cco"].values,
