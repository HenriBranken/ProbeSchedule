...
WEEKLY_BINNED_VERSION = True  # Alternative option is `None`.  Specifies whether to use weekly-binned kcp data or not.
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
# 2.    gaussian(x, amp=1, mean=0, sigma=1):
...
# 3.    weighted_moving_average(x, y, step_size=1, width=10):
...
def date_wrapper(dataframe):
    ...
    for d in dataframe.index:
        if BEGINNING_MONTH <= d.month <= 12:
            hacked_date = datetime.datetime(year=starting_year, month=d.month, day=d.day)
        else:
            hacked_date = datetime.datetime(year=starting_year + 1, month=d.month, day=d.day)
...
processed_df = pd.read_excel("./data/processed_probe_data.xlsx", header=0, index_col=0,
                             squeeze=False, parse_dates=["date"])
kcp_trend_df = pd.read_excel("./data/binned_kcp_data.xlsx", sheet_name="day_frequency", header=0, index_col=0,
...
if WEEKLY_BINNED_VERSION:
    projected_df = pd.read_excel("./data/projected_weekly_data.xlsx", header=0, index_col=0, parse_dates=True)
